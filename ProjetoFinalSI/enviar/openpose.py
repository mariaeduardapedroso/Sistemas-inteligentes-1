...
# Código usado para obtenção de dados por meio do OpenPose
...
# abre o arquivo onde serão salvos os dados, depois serão passados para o generateDataset
file = open("PosCair1.txt","w")
...
while cv.waitKey(1) < 0:
    ...
    # Pega os valores das posições requeridas se eles forem lidos
    ...
    # Estima a posição baseado na variação dos valores antigos dividido pela taxa de amostragem
    # Pescoço
    ...
    # Salva os dados obtidos no arquivo
    print(f"Neck: ", posNeck, " - RightHip: ", posRHip, " - LeftHip: ", posLHip)
    text = str(int(posNeck[0])) + "," + str(int(posNeck[1])) + "#" + str(int(posRHip[0])) + "," + str(int(posRHip[1])) + "#" + str(int(posLHip[0])) + "," + str(int(posLHip[1])) + "\n"
    file.write(text)
    oldNeck = posNeck
    oldLHip = posLHip
    oldRHip = posRHip

    cv.putText(frame, '%.2fms' % (t / freq), (500, 20), cv.FONT_HERSHEY_SIMPLEX, 0.5, (1, 1, 1))
